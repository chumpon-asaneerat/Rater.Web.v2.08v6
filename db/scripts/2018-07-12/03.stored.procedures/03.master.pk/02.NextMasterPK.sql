...
--
-- <Simple>
--exec NextMasterPK NULL, N'Customer';
--
-- <Complex>
...
DECLARE @iTblCnt tinyint;
	-- Error Code:
	--     0 : Success
	--  1101 : Table name cannot be null or empty string.
	--  1104 : Table name is not exists in MasterPK table.
	--  1105 : Not supports reset mode.
	--  1106 : Cannot generate seed code major cause should be 
	...
	BEGIN TRY
		...
		BEGIN
			EXEC GetErrorMsg @langId, 1104, @errNum out, @errMsg out
			RETURN;
		END
		...
		BEGIN
			...
			BEGIN
				-- not supports
				EXEC GetErrorMsg @langId, 1105, @errNum out, @errMsg out
				RETURN;
			END
			...
			ELSE
			BEGIN
				EXEC GetErrorMsg @langId, 1106, @errNum out, @errMsg out
                SET @errMsg = ' ' + @tableName + '.'
                RETURN;
			END
		END
		-- SUCCESS.
