...
function msSqldb(defFileName) {
    var result = nlib.loadJsonFile(path.join(dbdefPath, defFileName) + dbdefExt);
    //console.log(def);
...
msSqldb.prototype.Prepare = function (pObj, conn) {
    ...
    if (!nlib.isNull(medDef.inputs) && medDef.inputs.length > 0) {
        for (var i = 0; i < medDef.inputs.length; i++) {
            // each item.
            p_input = medDef.inputs[i];
            // get parameter name.
            p_name = p_input.name;
    ...
    if (!nlib.isNull(medDef.outputs) && medDef.outputs.length > 0) {
        for (var i = 0; i < medDef.outputs.length; i++) {
            // each item.
            p_output = medDef.outputs[i];
            // get parameter name.
            p_name = p_output.name;
            // get prefer sql data type.
...
msSqldb.prototype.ReadOutputs = function (conn, result) {
    ...
    if (!nlib.isNull(medDef.outputs) && medDef.outputs.length > 0) {
        for (var i = 0; i < medDef.outputs.length; i++) {
            var pName = medDef.outputs[i].name;

            if (pName === 'errNum') {
                // error No.
                if (!nlib.isNull(conn.parameters[pName])) {
                    result.errors.errNum = conn.parameters[pName].value;
                ...
                // error message.
                if (!nlib.isNull(conn.parameters[pName])) {
                    result.errors.errMsg = conn.parameters[pName].value;
                }
            }
                ...
                if (!nlib.isNull(conn.parameters[pName])) {
                    if (nlib.isNull(result.paging))
                        result.paging = {};
                    result.paging.pageNo = conn.parameters[pName].value;
                }
            }
                ...
                if (!nlib.isNull(conn.parameters[pName])) {
                    if (nlib.isNull(result.paging))
                        result.paging = {};
                    result.paging.rowPerPage = conn.parameters[pName].value;
                }
            }
                ...
                if (!nlib.isNull(conn.parameters[pName])) {
                    if (nlib.isNull(result.paging))
                        result.paging = {};
                    result.paging.totalRows = conn.parameters[pName].value;
                }
            }
                ...
                if (!nlib.isNull(conn.parameters[pName])) {
                    if (nlib.isNull(result.paging))
                        result.paging = {};
                    result.paging.maxPage = conn.parameters[pName].value;
                }
            }
            else {
                // other output parameter results.
                var pVal = conn.parameters[pName].value;
